...
int main()
{
    ...
    if ( FMA4::XMVerifyFMA4Support() )
    {
        ...
        XMVECTOR V = FMA4::XMVectorMultiplyAdd( c_V1, c_V2, c_V3 );
        printvector( L"V1*V2+V3", V );

        V = FMA4::XMVectorNegativeMultiplySubtract( c_V1, c_V2, c_V3 );
        printvector( L"V3-V1*V2", V );

        XMMATRIX M = FMA4::XMMatrixMultiply( c_M1, c_M2 );
        printvector( L"\nM1*M2[0]", M.r[0] );
        printvector( L"M1*M2[1]", M.r[1] );
        printvector( L"M1*M2[2]", M.r[2] );
        printvector( L"M1*M2[3]", M.r[3] );

        M = FMA4::XMMatrixMultiplyTranspose( c_M1, c_M2 );
        printvector( L"\nT(M1*M2[0])", M.r[0] );
        printvector( L"T(M1*M2[1])", M.r[1] );
        printvector( L"T(M1*M2[2])", M.r[2] );
        printvector( L"T(M1*M2[3])", M.r[3] );

        XMVECTOR VT2 = FMA4::XMVector2Transform( c_V1, M );
        printvector( L"\nVT2\n", VT2 );
        VT2 = FMA4::XMVector2TransformCoord( c_V1, M );
        printvector( L"coord", VT2 );
        VT2 = FMA4::XMVector2TransformNormal( c_V1, M );
        printvector( L"norm", VT2 );

        XMVECTOR VT3 = FMA4::XMVector3Transform( c_V2, M );
        printvector( L"\nVT3\n", VT3 );
        VT3 = FMA4::XMVector3TransformCoord( c_V2, M );
        printvector( L"coord", VT3 );
        VT3 = FMA4::XMVector3TransformNormal( c_V2, M );
        printvector( L"norm", VT3 );

        XMVECTOR VT4 = FMA4::XMVector3Transform( c_V3, M );
        printvector( L"\nVT4\n", VT4 );
    }
