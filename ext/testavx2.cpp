//-------------------------------------------------------------------------------------
// AVX2 extensions tester for SIMD C++ Math library
//
...
int main()
{
    if ( XMVerifyCPUSupport() )
    {
        ...
        V = XMVectorMultiplyAdd( c_V1, c_V2, c_V3 );
    ...
    if ( AVX2::XMVerifyAVX2Support() )
    {
        printf("AVX2 supported\n");

        XMVECTOR V = AVX2::XMVectorSplatX( c_A );
        printvector( L"A.xxxx", V );

        V = AVX2::XMVectorSplatY( c_A );
        printvector( L"A.yyyy", V );

        V = AVX2::XMVectorSplatZ( c_A );
        printvector( L"A.zzzz", V );

        V = AVX2::XMVectorSplatW( c_A );
        printvector( L"A.wwww", V );

        V = AVX2::XMVectorMultiplyAdd( c_V1, c_V2, c_V3 );
        printvector( L"V1*V2+V3", V );

        V = AVX2::XMVectorNegativeMultiplySubtract( c_V1, c_V2, c_V3 );
        printvector( L"V3-V1*V2", V );

        XMMATRIX M = AVX2::XMMatrixMultiply( c_M1, c_M2 );
        printvector( L"\nM1*M2[0]", M.r[0] );
        printvector( L"M1*M2[1]", M.r[1] );
        printvector( L"M1*M2[2]", M.r[2] );
        printvector( L"M1*M2[3]", M.r[3] );

        M = AVX2::XMMatrixMultiplyTranspose( c_M1, c_M2 );
        printvector( L"\nT(M1*M2[0])", M.r[0] );
        printvector( L"T(M1*M2[1])", M.r[1] );
        printvector( L"T(M1*M2[2])", M.r[2] );
        printvector( L"T(M1*M2[3])", M.r[3] );

        XMVECTOR VT2 = AVX2::XMVector2Transform( c_V1, M );
        printvector( L"\nVT2\n", VT2 );
        VT2 = AVX2::XMVector2TransformCoord( c_V1, M );
        printvector( L"coord", VT2 );
        VT2 = AVX2::XMVector2TransformNormal( c_V1, M );
        printvector( L"norm", VT2 );

        XMVECTOR VT3 = AVX2::XMVector3Transform( c_V2, M );
        printvector( L"\nVT3\n", VT3 );
        VT3 = AVX2::XMVector3TransformCoord( c_V2, M );
        printvector( L"coord", VT3 );
        VT3 = AVX2::XMVector3TransformNormal( c_V2, M );
        printvector( L"norm", VT3 );

        XMVECTOR VT4 = AVX2::XMVector3Transform( c_V3, M );
        printvector( L"\nVT4\n", VT4 );
    }
