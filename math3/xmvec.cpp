...
//XMVectorAbs 
    ...
    static const XMVECTORF32 v[] = {
        ...
        {-_INF,_INF,0,-.00000000001f}, {_INF,_INF,0,.00000000001f} 
    };
...
//XMVectorATan2 
...
#else
        const static float yval[] = { 0.f, -0.f,  0.f, -0.f, 0.f, -0.f,  0.f, -0.f, 1.f, -1.f, -1.f,   1.f,  -1.f, _INF, _INF, -_INF, -_INF }; 
        const static float xval[] = { 0.f, -0.f, -0.f,  0.f, 1.f,  1.f, -1.f, -1.f, 0.f,  0.f, _INF, -_INF, -_INF,  1.f, -1.f,   1.f,  -1.f };
#endif
    ...
    {
        const static float yval[]   = { _INF,    -_INF,        _INF,         -_INF,          1.f };
        const static float xval[]   = { _INF,    -_INF,        -_INF,        _INF,           _INF };
...
//XMVectorEqual 
    ...
    for(j = 0; j < 16; j++) {
        ...
        if(j & 8) { v1.v = XMVectorSetX(v1,XMVectorGetX(v2)+.01f); check.v = XMVectorSetIntX(check,0); } else {check.v = XMVectorSetIntX(check,(uint32_t)-1);}
        if(j & 4) { v1.v = XMVectorSetY(v1,XMVectorGetY(v2)+.01f); check.v = XMVectorSetIntY(check,0); } else {check.v = XMVectorSetIntY(check,(uint32_t)-1);}
        if(j & 2) { v1.v = XMVectorSetZ(v1,XMVectorGetZ(v2)+.01f); check.v = XMVectorSetIntZ(check,0); } else {check.v = XMVectorSetIntZ(check,(uint32_t)-1);}
        if(j & 1) { v1.v = XMVectorSetW(v1,XMVectorGetW(v2)+.01f); check.v = XMVectorSetIntW(check,0); } else {check.v = XMVectorSetIntW(check,(uint32_t)-1);}
...
//XMVectorGreater 
    ...
    for(j = 0; j < 16; j++) {
        ...
        if(j & 8) { v1.v = XMVectorSetX(v1,XMVectorGetX(v2)+.01f); check.v = XMVectorSetIntX(check,(uint32_t)-1); } else {check.v = XMVectorSetIntX(check,0);}
        if(j & 4) { v1.v = XMVectorSetY(v1,XMVectorGetY(v2)+.01f); check.v = XMVectorSetIntY(check,(uint32_t)-1); } else {check.v = XMVectorSetIntY(check,0);}
        if(j & 2) { v1.v = XMVectorSetZ(v1,XMVectorGetZ(v2)+.01f); check.v = XMVectorSetIntZ(check,(uint32_t)-1); } else {check.v = XMVectorSetIntZ(check,0);}
        if(j & 1) { v1.v = XMVectorSetW(v1,XMVectorGetW(v2)+.01f); check.v = XMVectorSetIntW(check,(uint32_t)-1); } else {check.v = XMVectorSetIntW(check,0);}
        ...
        if(j & 8) { v1.v = XMVectorSetX(v1,XMVectorGetX(v2)-.01f); check.v = XMVectorSetIntX(check,0); } else {check.v = XMVectorSetIntX(check,0);}
        if(j & 4) { v1.v = XMVectorSetY(v1,XMVectorGetY(v2)-.01f); check.v = XMVectorSetIntY(check,0); } else {check.v = XMVectorSetIntY(check,0);}
        if(j & 2) { v1.v = XMVectorSetZ(v1,XMVectorGetZ(v2)-.01f); check.v = XMVectorSetIntZ(check,0); } else {check.v = XMVectorSetIntZ(check,0);}
        if(j & 1) { v1.v = XMVectorSetW(v1,XMVectorGetW(v2)-.01f); check.v = XMVectorSetIntW(check,0); } else {check.v = XMVectorSetIntW(check,0);}
...
//XMVectorGreaterOrEqual 
    ...
    for(j = 0; j < 16; j++) {
        ...
        if(j & 8) { v1.v = XMVectorSetX(v1,XMVectorGetX(v2)+.01f); check.v = XMVectorSetIntX(check,(uint32_t)-1); } else {check.v = XMVectorSetIntX(check,(uint32_t)-1);}
        if(j & 4) { v1.v = XMVectorSetY(v1,XMVectorGetY(v2)+.01f); check.v = XMVectorSetIntY(check,(uint32_t)-1); } else {check.v = XMVectorSetIntY(check,(uint32_t)-1);}
        if(j & 2) { v1.v = XMVectorSetZ(v1,XMVectorGetZ(v2)+.01f); check.v = XMVectorSetIntZ(check,(uint32_t)-1); } else {check.v = XMVectorSetIntZ(check,(uint32_t)-1);}
        if(j & 1) { v1.v = XMVectorSetW(v1,XMVectorGetW(v2)+.01f); check.v = XMVectorSetIntW(check,(uint32_t)-1); } else {check.v = XMVectorSetIntW(check,(uint32_t)-1);}
...
//XMVectorReciprocal
    ...
    // Test special cases around 0, -0, +INF, -INF, _Q_NAN
    {
        ...
        const static float cval[] = { _INF, -_INF,  0.f,  -0.f, _Q_NAN };
#endif
...
#else
        const static float xval[] = {  0.f,     -0.f,   _INF,  -_INF, _Q_NAN };
        const static float cval[] = { _INF,    -_INF,      0, _Q_NAN, _Q_NAN };
#endif
...
HRESULT Test327(LogProxy* pLog)
{
    HRESULT ret = S_OK;
    XMVECTOR v;
    static const float f[] = { 0, 1, -.28f, _Q_NAN, _INF};
...
//XMVectorSaturate 
    ...
    int i;
    for(i = 0; i < sizeof(f) / sizeof(f[0]); i+=2) {
        XMVECTOR v1= f[i];
        XMVECTOR check = f[i+1];
...
//XMVectorSelectControl 
    static const XMVECTORF32 v[] = {{1,2,3,4}, {-10,-_INF,_Q_NAN,0}};
    HRESULT ret = S_OK;
    for(int k = 0; k < 16; k++) {
...
//XMVectorSqrt 
    ...
    // Test special cases around 0, -0, +INF, -INF
    {
        ...
        const static float cval[] = { 0.f, -0.f, _INF, _Q_NAN };
...
// The VMX128 implementation of the sqrt estimate doesn't handle -_INF consistent with the other implementations
#if defined(_XENON) && !defined(_XM_NO_INTRINSICS_)
        const static float cval[] = { 0.f, -0.f, _INF, -_INF};
#else
        const static float cval[] = { 0.f, -0.f, _INF, _Q_NAN };
#endif
...
//XMVectorSplatQNaN
    ...
    XMVECTOR check = XMVectorSet(_Q_NAN, _Q_NAN, _Q_NAN, _Q_NAN);
    COMPARISON c = CompareXMVECTOR(v,check,4);
    if(c != EXACT)
    {
        printe("%s: _Q_NAN _Q_NAN _Q_NAN _Q_NAN ... %f %f %f %f\n",
               TestName, XMVectorGetX(v),XMVectorGetY(v),XMVectorGetZ(v),XMVectorGetW(v));
        ret = MATH_FAIL; 
    }
    return ret;
...
//XMVectorSplatInfinity
    ...
    XMVECTOR check = XMVectorSet(_INF, _INF, _INF, _INF);
    COMPARISON c = CompareXMVECTOR(v,check,4);
    if(c != EXACT)
    {
        printe("%s: _INF _INF _INF _INF ... %f %f %f %f\n",
               TestName, XMVectorGetX(v),XMVectorGetY(v),XMVectorGetZ(v),XMVectorGetW(v));
        ret = MATH_FAIL; 
    }
    return ret;
...
//XMVECTOR operator /=, /, /=S, /S
...
#if !defined(_M_FP_FAST) || !defined(_XM_NO_INTRINSICS_)
    {
        ...
        const static float cval[] = { _INF, -_INF, 0.0f, -0.0f, /*-_INF, _INF,*/   0.0f, -0.0f };
        ...
        for(int i = 0; i < sizeof(xval)/sizeof(float); ++i )
        {
            ...
            if ( c > EXACT )
            {
                printe("%s: %f/%f (%d)= %f %f %f %f...%f %f %f %f %d\n", TestName, y, x, i,
                       XMVectorGetX(r),XMVectorGetY(r),XMVectorGetZ(r),XMVectorGetW(r),
                       XMVectorGetX(check),XMVectorGetY(check),XMVectorGetZ(check),XMVectorGetW(check), c);
                ret = MATH_FAIL;
            }
